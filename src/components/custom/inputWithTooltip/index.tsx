...
interface TextAreaWithTooltipProps {
  ...
  const TextAreaWithTooltip2: React.FC<TextAreaWithTooltipProps> = ({
    ...
    const { localValue, setLocalValue, count, warning } = useCharacterCount(state, wordLimit);
    ...
    const addTooltip = () => {
      if (highlightedText && contentEditableRef.current) {
        ...
        setLocalValue(newState); // Update the hook state
    ...
    const handleInput = () => {
        if (contentEditableRef.current) {
          ...
          if (textContent.length <= wordLimit) {
            setLocalValue(textContent); // Update state if within limit
    ...
    const updateTooltipText = (index: number, newText: string) => {
      ...
      if (tooltip.type === "tooltip") {
        ...
        setLocalValue(updatedHTML);
      }
    };
  
    useEffect(() => {
      if (contentEditableRef.current && contentEditableRef.current.innerHTML !== localValue) {
          ...
          <div>
            <PushSheet side="right">
              <PushSheetTrigger isOpen={undefined} onToggle={undefined}>
                <Button variant={"outline"}>Manage Tooltips</Button>
              </PushSheetTrigger>
              <PushSheetHeader>
