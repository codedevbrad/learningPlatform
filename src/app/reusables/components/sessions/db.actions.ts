...
import { db_fetchUser, getUserByUserId } from '@/db_queries/user/queries';
...
export async function db__confirmProposedSession({ studentId }: DbConfirmProposedSessionParams): Promise<Sessions | null> {
  try {
    ...
    // get userId from student using the studentId...
    const { userId } = await getUserByUserId(studentId);

    if ( !user ) throw new Error('Couldnt locate userId of student');
    return userId;
