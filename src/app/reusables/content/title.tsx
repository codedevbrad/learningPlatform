...
const Title: React.FC<TitleProps> = ({ title = 'Title not set', variant = 'heading' , noMargin = false , className }) => {
  ...
  const getFontSize = (): string => {
    switch (variant) {
      ...
      case 'subheading3': 
        return 'font-bold text-md'; // Subheading3 style
  ...
  return (
    <h2 className={ `${getFontSize()} ${ noMargin ? 'py-1' : 'py-3'} ${ className }`}>{title}</h2>
  );
};
